# !/bin/bash

GREEN='\033[1;32m'
RED='\033[0;31m'
NC='\033[0m' # No Color

THRESHOLD=2/3
show_spinner()
{
  local -r pid="${1}"
  local -r delay='0.75'
  local spinstr='\|/-'
  local temp
  while ps a | awk '{print $1}' | grep -q "${pid}"; do
    temp="${spinstr#?}"
    printf " [%c]  " "${spinstr}"
    spinstr=${temp}${spinstr%"${temp}"}
    sleep "${delay}"
    printf "\b\b\b\b\b\b"
  done
  printf "    \b\b\b\b"
}

if [[ -z "${BC_ADMIN_ID}" ]]; then
  echo "${RED}You need to define the administrator key as env variable before running this script!${NC}"
  echo "export BC_ADMIN_ID=ed25519:3b9637....."
  exit 1
fi

if [[ -z "${BC_ADMIN_ID}" ]]; then
  echo "${RED}You need to define the Byzcoin config path as env variable before running this script!${NC}"
  echo 'export BC="/path/to/.../bc-ce98bc8ac...bed4038.cfg"'
  exit 1
fi

echo "${GREEN}"
echo "" | base64 --decode
echo  "${NC}"

echo "${GREEN}## setting up rules for evolving the administration darc ##${NC}"
echo "Adding rule invoke:darc.evolve"
bcadmin darc rule --rule invoke:darc.evolve -id "threshold<${THRESHOLD},${BC_ADMIN_ID}>" --replace &
show_spinner "$!"
echo


echo "${GREEN}## setting up rules for spawning deferred transactions ##${NC}"
echo "Adding rule spawn:deferred"
bcadmin darc rule --rule spawn:deferred -id "${BC_ADMIN_ID}" --replace &
show_spinner "$!"
echo "Adding rule invoke:deferred.addProof"
bcadmin darc rule --rule invoke:deferred.addProof -id "${BC_ADMIN_ID}" --replace &
show_spinner "$!"
echo "Adding rule invoke:deferred.execProposedTx"
bcadmin darc rule --rule invoke:deferred.execProposedTx -id "${BC_ADMIN_ID}" --replace &
show_spinner "$!"
echo

echo "${GREEN}## setting up rules for spawning projects contracts ##${NC}"
echo "Adding rule spawn:project"
bcadmin darc rule --rule spawn:project -id "threshold<${THRESHOLD},${BC_ADMIN_ID}>" --replace &
show_spinner "$!"
echo "Adding rule invoke:project.add"
bcadmin darc rule --rule invoke:project.add -id "threshold<${THRESHOLD},${BC_ADMIN_ID}>" --replace &
show_spinner "$!"
echo "Adding rule invoke:project.remove"
bcadmin darc rule --rule invoke:project.remove -id "threshold<${THRESHOLD},${BC_ADMIN_ID}>" --replace &
show_spinner "$!"